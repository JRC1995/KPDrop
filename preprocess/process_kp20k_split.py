...
Path('../processed_data/kp20k_big_unsup/').mkdir(parents=True, exist_ok=True)
Path('../processed_data/kp20k_low_res/').mkdir(parents=True, exist_ok=True)

train_save_path_big_unsup = Path('../processed_data/kp20k_big_unsup/train.jsonl')
dev_save_path_big_unsup = {}
for key in dev_keys:
    dev_save_path_big_unsup[key] = Path('../processed_data/kp20k_big_unsup/dev_{}.jsonl'.format(key))
test_save_path_big_unsup = {}
for key in test_keys:
    test_save_path_big_unsup[key] = Path('../processed_data/kp20k_big_unsup/test_{}.jsonl'.format(key))
metadata_save_path_big_unsup = fspath(Path("../processed_data/kp20k_big_unsup/metadata.pkl"))

train_save_path_low_res = Path('../processed_data/kp20k_low_res/train.jsonl')
dev_save_path_low_res = {}
for key in dev_keys:
    dev_save_path_low_res[key] = Path('../processed_data/kp20k_low_res/dev_{}.jsonl'.format(key))
test_save_path_low_res = {}
for key in test_keys:
    test_save_path_low_res[key] = Path('../processed_data/kp20k_low_res/test_{}.jsonl'.format(key))
metadata_save_path_low_res = fspath(Path("../processed_data/kp20k_low_res/metadata.pkl"))
...
idx = [id for id in range(len(train_srcs))]
random.shuffle(idx)

idx1 = idx[0:-5000]
idx2 = idx[-5000:]

train_srcs1 = [train_srcs[id] for id in idx1]
train_trgs1 = [train_trgs[id] for id in idx1]
train_trgs_no_peos1 = [train_trgs_no_peos[id] for id in idx1]

train_srcs2 = [train_srcs[id] for id in idx2]
train_trgs2 = [train_trgs[id] for id in idx2]
train_trgs_no_peos2 = [train_trgs_no_peos[id] for id in idx2]
...
train_data = vectorize_data(train_srcs1, train_trgs1, train_trgs_no_peos1)
jsonl_save(filepath=train_save_path_big_unsup,
           data_dict=train_data)

train_data = vectorize_data(train_srcs2, train_trgs2, train_trgs_no_peos2)
jsonl_save(filepath=train_save_path_low_res,
           data_dict=train_data)

dev_data = {}
for key in dev_keys:
    dev_data[key] = vectorize_data(dev_srcs[key], dev_trgs[key], dev_trgs_no_peos[key])
    jsonl_save(filepath=dev_save_path_big_unsup[key],
               data_dict=dev_data[key])

test_data = {}
for key in test_keys:
    test_data[key] = vectorize_data(test_srcs[key], test_trgs[key], test_trgs_no_peos[key])
    jsonl_save(filepath=test_save_path_big_unsup[key],
...
with open(metadata_save_path_big_unsup, 'wb') as outfile:
...
for key in dev_keys:
    dev_data[key] = vectorize_data(dev_srcs[key], dev_trgs[key], dev_trgs_no_peos[key])
    jsonl_save(filepath=dev_save_path_low_res[key],
               data_dict=dev_data[key])

test_data = {}
for key in test_keys:
    test_data[key] = vectorize_data(test_srcs[key], test_trgs[key], test_trgs_no_peos[key])
    jsonl_save(filepath=test_save_path_low_res[key],
               data_dict=test_data[key])
...
with open(metadata_save_path_low_res, 'wb') as outfile:
    pickle.dump(metadata, outfile)
