...
dev_keys = ["kp20k2"]
test_keys = ["kp20k2", "krapivin", "inspec", "semeval", "nus"]

train_src_path = Path('../dataset/kp20k_sorted/train_src.txt')
train_trg_path = Path('../dataset/kp20k_sorted/train_trg.txt')

dev_src_path = {}
dev_trg_path = {}
dev_src_path["kp20k2"] = Path('../dataset/kp20k_sorted/valid_src.txt')
dev_trg_path["kp20k2"] = Path('../dataset/kp20k_sorted/valid_trg.txt')

test_src_path = {}
test_trg_path = {}
test_src_path["kp20k2"] = Path('../dataset/kp20k_sorted/test_src.txt')
test_trg_path["kp20k2"] = Path('../dataset/kp20k_sorted/test_trg.txt')
...
Path('../processed_data/kp20k2/').mkdir(parents=True, exist_ok=True)

train_save_path = Path('../processed_data/kp20k2/train.jsonl')
dev_save_path = {}
for key in dev_keys:
    dev_save_path[key] = Path('../processed_data/kp20k2/dev_{}.jsonl'.format(key))
test_save_path = {}
for key in test_keys:
    test_save_path[key] = Path('../processed_data/kp20k2/test_{}.jsonl'.format(key))
metadata_save_path = fspath(Path("../processed_data/kp20k2/metadata.pkl"))
...
special_tags = [";", "<sos>", "<unk>", "<eos>", "<pad>", "<digit>"]
...
def reorg_trg(trg, trg_vec, src):
    ...
    for token_id, token in zip(trg_vec, trg):
        if token == "<peos>":
            ...
            ptr_token_id = None
            if token_id == UNK_id:
                ptr_token_id = src_token_dict.get(token, UNK_id)
