...
def load_data(paths, metadata, args, config):
    ...
    if data["vocab2idx"] is not None:
        ...
        config["SOS_id"] = vocab2idx["<sos>"]
        if config["one2set"]:
            config["NULL_id"] = vocab2idx["<null>"]
        ...
        config["idx2vocab"] = {id: token for token, id in vocab2idx.items()}
        config["vocab2idx"] = vocab2idx
        config["UNK_id"] = data["UNK_id"]
        config["SEP_id"] = data["SEP_id"]
        config["PAD_id"] = data["PAD_id"]
        config["vocab_len"] = data["vocab_len"]
    else:
        data["PAD_id"] = None
        data["SEP_id"] = None
        data["UNK_id"] = None
    ...
    if config["use_dev_as_test"]:
        data["test"] = {key: load_dataset(paths["dev"][key], limit=args.limit) for key in paths["dev"]}
    else:
